...
Use these documents to navigate the project:

- [Project overview](../docs/project-overview.md): Architecture, tech stack, and project structure.
...
- **Systematic Debugging:** Use test failures as guides to the underlying issue.
...
The combination of fast unit and integration feedback with end-to-end tests provides confidence in complete user workflows.

### Testing Approach by Context

- **Backend API changes:** Write Jest tests first, then implement using the RED-GREEN-REFACTOR cycle.
- **Frontend component features:** Write React Testing Library tests first for component behavior, then implement. Follow with manual browser testing for complete UI flows.
...
GitHub CLI commands for workflow automation are available in all modes:

- List open issues: `gh issue list --state open`
- Get issue details: `gh issue view <issue-number>`
- Get an issue with comments: `gh issue view <issue-number> --comments`

The main exercise issue has `Exercise:` in its title, and its steps are posted as comments. Use these commands when `/execute-step` or `/validate-step` prompts are invoked.
